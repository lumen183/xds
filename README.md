...
Ascend 真机的快速 smoke 测试、性能测试和脚手架设计见
...
申请 Ascend 内存，也不会加载内核模块。已准备好的 Ascend 目标机可通过统一脚手架
执行实际 P2P I/O：

```bash
./tools/xds.sh setup
./tools/xds.sh smoke --bdev /dev/nvme0n1 --data-dir /mnt/nvme --size 8M
./tools/xds.sh bench --bdev /dev/nvme0n1 --data-dir /mnt/nvme --size 1G \
  --api single --inflight 8 --warmup 5 --iterations 100 --json result.json
./tools/xds.sh cleanup
```

`setup` 仅在需要时通过 `sudo insmod` 加载模块；`cleanup` 不会强制卸载仍被使用的
模块。`smoke` 和 `bench` 不会降级为 mock，默认生成确定性数据、验证 FIEMAP 与块设备
对应关系，并在完成后删除生成的临时文件。完整参数和性能计时边界见
[docs/TEST_GUIDE.md](docs/TEST_GUIDE.md)。
