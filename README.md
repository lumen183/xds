...
如果连续组合满载会触发驱动、供电或温度保护，可对每个组合强制隔离并留出冷却时间：

```bash
python3 tools/single_npu_stream_bench.py \
  --bdev /dev/nvme0n1 --data-dir /mnt/nvme \
  --isolate-tests --inter-test-delay 10 --batch-delay 0.01
```

`--isolate-tests` 为每个 `size × io-depth` 组合重新创建 P2P fd，组合结束后执行 NPU 同步、
关闭 fd、Python GC 和 NPU cache 清理。`--inter-test-delay`（秒）在组合之间暂停；如果单个
组合内部仍会持续满载，`--batch-delay`（秒）会在相邻 `drain_read()` batch 之间节流，并且
同样作用于独立校验 pass。批次休眠不计入 `bandwidth_bytes_per_sec` 的 active 时间；JSON
同时记录 `elapsed_ns`、`wall_elapsed_ns` 和 `throttle_sleep_ns`，避免隐藏真实墙钟耗时。
...
C++ runner 支持与 Python runner 同名的三项安全控制。`--isolate-tests` 为每个组合创建独立
P2P fd；计时扫描和样本校验的 ACL buffer 离开作用域后，runner 会执行
`aclrtSynchronizeDevice()` 再关闭该 fd。`--inter-test-delay` 在组合之间冷却，
`--batch-delay` 在计时扫描的相邻 `drain_read()` 之间以及三个样本校验之间暂停。建议在问题
机器上把这三项一起启用，并从较长延迟开始逐步下调，不要直接运行无节流的完整默认矩阵。

和 Python runner 一样，C++ JSON 会记录顶层的 `isolate_tests`、`inter_test_delay`、
`batch_delay`，以及每个结果的 active `elapsed_ns`、`wall_elapsed_ns` 和
`throttle_sleep_ns`。主动休眠不计入带宽，但墙钟时间会完整保留。以上三个安全参数都以秒为
单位，接受小数；负数、NaN 和无穷大会在初始化 ACL 或访问设备之前被拒绝。
...
| 多张 NPU 并发读取 | 有实验性双卡 runner，尚未完成真机回归 | `tools/dual_npu_dual_nvme_bench.py` 为每张卡创建独立进程、tensor、测试文件和 `/dev/p2p_device` fd，并执行同步起跑、逐卡校验和聚合吞吐统计。它用于验证已知拓扑，不代表任意多卡/多盘组合已受支持。 |
...
仓库已新增实验性双卡 runner，默认对应 NPU0 + `/dev/nvme3n1`（`/workspace`）和
NPU1 + `/dev/nvme4n1`（`/data`）：

```bash
python3 tools/dual_npu_dual_nvme_bench.py \
  --request-size 128K --io-depth 32 \
  --warmup 2 --iterations 20 \
  --json dual-npu-results.json --verbose
```

当前目标驱动实测 NPU0/NPU1 地址都通过 SVM context 0 查询，因此默认计算卡号分别为
